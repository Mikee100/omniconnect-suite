...
import { MessageSquare, Users, Clock, TrendingUp, Send, Inbox } from 'lucide-react';
import { instagramApi } from '@/api/messaging';
import {
...
const InstagramTab = () => {
  ...
  const loadStats = async () => {
    try {
      const data = await instagramApi.getStats();
      setStats(data);
    } catch (error) {
  ...
  const messageDirectionData = [
    { name: 'Inbound', value: stats.inboundMessages, color: '#8b5cf6' },
